<template>
  <scroll class="list-view" :data="data" ref="listView" :listScroll="listScroll" @scroll="scroll" :probeType="probeType">
    <ul>
    ...
    <div class="list-shortcut"
         ...
         >
      <ul>
        <li v-for="(item, index) in shortcutList" class="item" :data-index=index :class="{'current': index === currentIndex}">{{item}}</li>
      </ul>
    </div>
...
export default{
  ...
  methods: {
    ...
    onShortcutTouchMove (e) {
      ...
      let delta = (this.touch.y2 - this.touch.y1) / ANCHOR_INDEX | 0
  ...
  watch: {
    ...
    scrollY (newY) {
      ...
      // 当滚动到最后的时候
      this.currentIndex = list.height - 2
    },
    diff (newVal) {
